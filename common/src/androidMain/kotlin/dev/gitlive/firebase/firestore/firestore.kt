...
actual class FirebaseFirestore(val android: com.google.firebase.firestore.FirebaseFirestore) {
    ...
    actual fun collection(path: String) = CollectionReference(android.collection(path))
...
//    }
    ...
    actual suspend fun set(data: Any) {
        android.set(data)
    }
}
...
//    internal actual fun _startBefore(document: DocumentSnapshot) = Query(android.startAfter(document.android))
...
//        get() = android.path
    ...
    actual suspend fun document(path: String): DocumentReference = DocumentReference(android.document(path))
...
//        DocumentReference(android.add(encode(strategy, data, encodeDefaults)!!).await())
    actual suspend fun <T> add(document: T, encoder: (T) -> Any): DocumentReference =
        DocumentReference(android.add(encoder(document)).await())
...
//@Suppress("UNCHECKED_CAST")
actual open class DocumentSnapshot(open val android: com.google.firebase.firestore.DocumentSnapshot) {
//
...
//    }
    /**
     * decoder is not used on android clients
     */
    internal actual inline fun <reified T> data(decoder: DocumentSnapshot.() -> T?): T? = android.toObject<T>()
    actual fun get(fieldPath: String): Any? = android.get(fieldPath)
//    internal actual fun <T> data(decoder: (Map<String, Any>) -> T): T? = android.data?.let { decoder(it) }
}
...
actual class QueryDocumentSnapshot<T>(override val android: com.google.firebase.firestore.QueryDocumentSnapshot): DocumentSnapshot(android) {
    //actual inline fun <reified T> data(): T = android.toObject()
